 ...
 *
 * This function does NOT require any auth — it is triggered exclusively by
 * Supabase's pg_cron scheduler (or an external cron) and therefore runs with
 * the service role key stored in the function's environment.
 *
 * Deploy this as an HTTP-triggered function and configure the cron to call it
 * with:
 *   Authorization: Bearer <SUPABASE_SERVICE_ROLE_KEY>
 *
 * The Supabase dashboard cron scheduler can use a native DB cron instead;
...
Deno.serve(async (req) => {
  // Only accept requests that carry the service-role key as a bearer token
  // so this endpoint cannot be invoked by arbitrary callers.
  ...
  const results = await Promise.allSettled(
    expiredRules.map(async (rule) => {
      ...
      // 2c. Insert an activity log entry so the parent dashboard shows the expiry
      ...
      if (logError) {
        // Non-fatal — rule was already deleted; just log it
